...
{	
	...
	function sendMail($args)
	{
		...
		if ($sender_email && $sender_name && $receive_email && $email_subject) {			
			
			include_once $this -> GP -> CLS . 'class.mail.php';
			$C_SendMail = new SendMail();
			
			$C_SendMail -> setUseSMTPServer(false);
			$C_SendMail -> setSMTPServer($this->GP -> SMTP_SERVER, $this->GP -> SMTP_PORT);
			...
			$C_SendMail -> addTo($receive_email);
	...
	function membersFindInfo ($args) {
		...
		//탈퇴 회원은 제외
		$qry = "Select mb_code, mb_email, mb_name From tblMember Where mb_id = '$mb_id' and replace(mb_email,'@','') = '$mb_email' And mb_del_flag = 'N' and mb_status='M' ";
