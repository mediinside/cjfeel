...
CLASS Hold extends Dbconn
{
	...
	function Main_Slide_Show($args='') {
        ...
        $addQry = '';
		if($ts_gubun != ''){
			$addQry .=" and ts_gubun = '$ts_gubun' ";
		}
		
		$qry = "
			select * from tblHold where ts_show ='Y' $addQry order by ts_regdate desc $limit
		";		
	...
	function Slide_Del($args = '') {
		...
		$qry = "
			delete from tblHold where ts_idx = '$ts_idx'	
		";
		$rst =  $this -> DB -> execSqlUpdate($qry);
		return $rst;
	}
	...
	function Slide_Info($args = '') {
		...
		$qry = "
			select * from tblHold where ts_idx = '$ts_idx'
		";
