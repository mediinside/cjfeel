...
CLASS JHBoard extends Dbconn
{
	...
	function Board_List($args = '') {
		...
		if($list_view == "ALL") {
			$addQry = "(A.jb_code='10'|| A.jb_code='20') and A.jb_del_flag = 'N'";
		}else{
		...
		if($jb_owner_id != '') {
			$addQry .= " and A.jb_owner_id='$jb_owner_id' ";
		}
	...
	//	if($jb_code == '50') $args['pagetype'] = "sc";
		...
		*/
		...
		if($list_view == "ALL") {
			$order_q = "A.jb_order ASC, A.jb_etc3 desc, A.jb_reg_date desc, A.jb_depth DESC";
		}else{
			// if($jb_code == 60 || $jb_code == 10 || $jb_code == 20 || $jb_code == 30 || $jb_code == 70){
			// 	$order_q = "A.jb_order ASC, A.jb_etc3 desc";
			// }else{
