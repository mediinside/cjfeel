...
if($data_list_cnt > 0) {	
	for ($i=0; $i<$data_list_cnt; $i++ ) {
		...
		//	$jb_name 						= substr($data_list[$i]['jb_name'],0,-3)."*";
			...
			$jb_show				= $data_list[$i]['jb_show'];
			$jb_age					= $data_list[$i]['jb_age'];
			$jb_gender				= $data_list[$i]['jb_gender'];
			...
			$jb_content 			= $C_Func->strcut_utf8($jb_content, 150, true, "...");						//제목 (길이, HTML TAG제한여부 처리)
		...
		//	$jb_title = $depth_icon . $jb_title . $comment_count . $new_icon; //이미지타이틀 처리
			...
			if($jb_front_image != '') {
				...
				$img_src = "<img src='/public/images/no-image.jpg' alt='이미지 없음' class='block' >";
			}
	...
	/*		
			//공지글여부
			if($jb_order < 100) {
				$jb_no ="<div class='review best'>";
			}else{
				$jb_no ="<div class='review'>";
			}
			
			
			if($jb_show =="A"){
				$jb_no ="<div class='review secret'>";
			}
	*/		
			...
			echo ("
					<li>
						<a href='" . $get_par . "'>
							<h3 class='tit'>${jb_title}</h3>
							<p class='cont'>${jb_content}</p>
							<div class='exp-text'>
								<dl class='exp-info'>
									<dt class='none'>나이</dt>
									<dd>${jb_age}</dd>
									<dt class='none'>이름</dt>
									<dd>${jb_name}</dd>
									<dt class='none'>원장님</dt>
									<dd>${jb_etc1} 원장님</dd>
								</dl>
								<div class='exp-num'>
									<span class='num'>NO." . $num_idx--. "</span>
									<span class='date'>${jb_etc3}</span>
								</div>
							</div>
						</a>
					</li>
