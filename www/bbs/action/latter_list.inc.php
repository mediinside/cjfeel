...
if($data_list_cnt > 0) {
	for ($i=0; $i<$data_list_cnt; $i++ ) {		
			...
			$jb_major 						= $data_list[$i]['jb_major'];
			$jb_snum 						= $data_list[$i]['jb_snum'];
			$jb_type2 						= $data_list[$i]['jb_type2'];
			...
			$new_icon = $C_Func->new_icon(7, $data_list[$i]['jb_reg_date'], $new_image);
			...
			if($jb_comment_count > 0) {
				$comment_count = "<span class='reply'>[${jb_comment_count}]</span>";
			}else{
				$comment_count = "";
			}
			...
			echo ("
					<li>
						<a href='" . $get_par . "'>
							<span class='num'><strong>NO.</strong> " . $num_idx--. "</span>
							<span class='subject'><strong>제목</strong> <span class='txt'>${jb_title}</span></span>
							<span class='count'><strong>구분</strong> ${jb_type2}</span>
							<span class='name'><strong>회원정보</strong> ${jb_name}(${jb_major}${jb_snum})</span>
							<span class='date'><strong>작성일</strong> ${jb_reg_date}</span>
						</a>
					</li>
				");
			$dummy++;
