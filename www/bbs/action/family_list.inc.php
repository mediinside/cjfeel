...
if($data_list_cnt > 0) {	
	for ($i=0; $i<$data_list_cnt; $i++ ) {		
			...
			$jb_etc3					= stripslashes($data_list[$i]['jb_etc3']);
			$jb_etc3 					= $C_Func->strcut_utf8($jb_etc3, 100, true, "...");
			...
			if($jb_etc1 != '') {
				$unit = " / " . $GP -> HP_UNIT[$jb_etc1];
			}

			if($jb_etc2 != '') {
				$unit .= " " . $GP -> HP_UNIT_DEPTH1[$jb_etc1][$jb_etc2];
			}
			
			$jb_title = $depth_icon . $jb_title . $unit . $comment_count . $new_icon; //이미지타이틀 처리
			...
			if($jb_file_code != '') {
				...
				$img_src = "<img src='/images/no_image.jpg' alt='이미지 없음' >";	
			}
