	...
	for ($i=0; $i<$data_list_cnt; $i++ ) {		
			...
			if($check_level < 9 && $data_list[$i]['jb_secret_check'] == "Y")
			{
				...
				if($jb_mb_id == "") {
					...
					$get_par.= "&jb_mode=tsecret&month=$month&type=$type";				
					...
					if($check_id != $jb_mb_id) {
						...
						if (trim($check_id) == "") {
							...
							$get_par = "${index_page}?jb_code=${jb_code}&jb_idx=${jb_idx}";
							$get_par.="&search_key=" . $_GET['search_key'] . "&search_keyword=${search_keyword}&page=${page}&month=$month&type=$type";
						} else {						
			...
			if($jb_file_code != '') {
				...
				$img_src = "<img src='/images/hu_logo_img.jpg' alt='이미지 없음' >";	
			}
			...
			include_once($GP->CLS."class.schedule.php");
			$C_Schedule 	= new Schedule;
			$args = '';
			$args['sch_idx'] = $sch_idx;
			$sch_data = $C_Schedule ->SCH_Info($args);
			...
			echo ("
					<li>
						<a href='" . $get_par . "'>
							...
							<strong class='tit'>[" . $GP -> EDU_TYPE[$sch_edu_type] . "]</strong>
							<span class='txt'>${jb_title}</span>
						</a>
					</li>				
				");
