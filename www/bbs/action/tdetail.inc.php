...
if($check_level < $db_config_data['jba_read_level']) {
	$C_Func->put_msg_and_go("로그인이 필요한 메뉴입니다.", "/member/login.html?re_url=".$GP -> NOWPAGE);
	die;
}
...
if($jb_code == 20 && $check_level < 9){
	$mypage_id =  $_SESSION['suserid'];
}
...
*/	
	$content	= nl2Br($C_Func->dec_contents_view($jb_content));				
	//$pattern = "/(<img.*?)/i"; 
	...
	$get_par.= "&${search_key}&search_keyword=$search_keyword&page=${page}&month=$month&type=$type";
	...
	if($be_idx != '') {
		...
		$get_par1.= "&${search_key}&search_keyword=$search_keyword&page=${page}&month=$month&type=$type";
	}	
	
	//다음글
	if($af_idx != '') {
		$get_par2 = "${index_page}?jb_code=${jb_code}&jb_idx=${af_idx}&jb_group=${jb_group}&jb_step=${jb_step}&jb_depth=${jb_depth}";
		$get_par2.= "&${search_key}&search_keyword=$search_keyword&page=${page}&month=$month&type=$type";
	}
