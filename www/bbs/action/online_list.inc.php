...
*/				
			...
			if($check_level < $db_config_data['jba_reply_level']  && $data_list[$i]['jb_secret_check'] == "Y")
			{
				...
				if($jb_mb_id == "") {
					...
					if($check_id != $jb_mb_id) {
						...
						//	$get_par.= "&jb_mode=tsecret";
							$get_par ='javascript:alert("자신이 작성한 글과 답변만 읽을 수 있습니다.");';
		...
		if($jb_comment_count < 1 )	{
			...
			if($jb_secret_check == "Y") {
				$secret_icon = '<span class="lock"></span>';
			}else{
				$secret_icon = '<span class="unlock"></span>';
			}
			...
			$jb_title =  $depth_icon . $jb_title . $comment_count . $secret_icon . $new_icon; //이미지타이틀 처리
			

			if($jb_show =="A"){
				$jb_no ="<td class='num'>숨김</td>";
			}
...
*/			
			if($jb_mb_level >= 9) {
				$jb_gubun ="<td class='state'></td>
							<td class='subject reply'><span class='reply'>답변</span>";
			}else {
				$jb_gubun ="<td class='num'>" . $num_idx--. "</td>
							<td class='subject'>";
			}
	...
	/*		
			//파일 여부
			if($jb_file_code != ''){
				$jb_title = $depth_icon . $jb_title . $comment_count.$new_file;
			}
	...
	*/	
			...
			echo ("
						<tr>
							${jb_gubun}
							<a href='" . $get_par . "'>${jb_title}</a></td>
							<td class='name'>${jb_name}</td>
