...
if($data_list_cnt > 0) {
	for ($i=0; $i<$data_list_cnt; $i++ ) {		
		...
		//	$jb_content 				= $data_list[$i]['jb_content'];	//제목 (길이, HTML TAG제한여부 처리)
		...
		//	$new_image = "<span class='new'>new</span>";
			...
			if($jb_file_code != ''){
				...
				$new_file = "<span class='icon-file'></span>";
			}
			//댓글 수
			...
			$jb_title = $depth_icon . $jb_title . $comment_count . $new_icon . $new_file; //이미지타이틀 처리

			//공지글여부
			$jb_notice = $jb_order < 100 ? " class='notice'" : "";
			$jb_no = $jb_order < 100 ? "" : $num_idx--;

			echo ("
					<li>
						<a href='" . $get_par . "' class='acco-trigger'>${jb_title}</a>
						<div class='acco-contents'>
							${jb_content}
						</div>
					</li>
				");
